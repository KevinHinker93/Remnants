...
#include "TripleShotModule.h"

UTripleShotModule::UTripleShotModule()
{
	m_rotOffset = 1.2f;
...
void UTripleShotModule::FiringLogic(AActor* shooter, const FVector spawnLoc, const FRotator spawnRot, UWorld* const world,
		int damage, float damageMult, float firingSpeed, float firingSpeedMult, UAElement* element, FName shooterType)
{
	...
	//	if (!projClass)
	//	{
	//		UE_LOG(LogTemp, Error, TEXT("Triple Shot Module has no proj to fire"));
	//		return;
	//	}
...
void UTripleShotModule::StopFiring(UWorld* world)
{
